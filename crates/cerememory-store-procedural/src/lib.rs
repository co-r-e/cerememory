//! Cerememory procedural store implementation backed by redb.
//!
//! This crate provides a persistent, file-based procedural memory store using
//! [redb](https://docs.rs/redb) as the storage engine. All records are serialized
//! via MessagePack (`rmp-serde`) and indexed by fidelity for efficient
//! decay-based sweeps.
//!
...
const PROCEDURAL_RECORDS: TableDefinition<&[u8], &[u8]> =
    TableDefinition::new("procedural_records");

/// Fidelity index: (fidelity_bucket_u8 ++ record_id) → ().
/// Bucket = (fidelity.score * 100).round() clamped to [0, 100].
const PROCEDURAL_FIDELITY_INDEX: TableDefinition<&[u8], ()> =
    TableDefinition::new("procedural_fidelity_index");
...
/// A redb-backed procedural memory store.
///
...
impl ProceduralStore {
    ...
    /// Create an ephemeral in-memory store (backward-compatible alias).
    pub fn new() -> Self {
        Self::open_in_memory().expect("failed to create in-memory ProceduralStore")
    }

    /// Ensure both tables exist by performing a write transaction.
...
mod tests {
    ...
    fn temp_store() -> ProceduralStore {
        ProceduralStore::open_in_memory().expect("failed to create in-memory store")
    }

    /// Helper: create a simple text record.
    fn make_record(text: &str) -> MemoryRecord {
        MemoryRecord::new_text(StoreType::Procedural, text)
    }
    ...
    async fn crud_roundtrip() {
        ...
        // Store
        let record = make_record("Hello, procedural memory!");
        ...
        assert_eq!(retrieved.text_content(), Some("Hello, procedural memory!"));
    ...
    async fn msgpack_integrity() {
        ...
        assert_eq!(retrieved.store, StoreType::Procedural);
    ...
    // 3. Persistence (open, write, close, reopen, read)
    #[tokio::test]
    ...
    async fn limit_zero() {
        ...
        store.store(make_record("Some record")).await.unwrap();

        let results = store.query_text("Some", 0).await.unwrap();
        assert!(results.is_empty());
    }
    ...
    async fn missing_record_error() {
        ...
        // update_fidelity on missing record returns error
        let result = store
            .update_fidelity(&fake_id, FidelityState::default())
            .await;
        assert!(result.is_err());

        // update_record on missing record returns error
        let result = store.update_record(&fake_id, None, None, None).await;
        assert!(result.is_err());

        // update_access on missing record returns error
        let result = store.update_access(&fake_id, 0, Utc::now()).await;
        assert!(result.is_err());
    }
}
